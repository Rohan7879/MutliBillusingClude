...
async function loadBillsInRange() {
  ...
  try {
    ...
    if (startVal && endVal) {
      ...
      const e = new Date(endVal); e.setHours(23,59,59);
...
function showSummary(items) {
  document.getElementById("report-summary").innerHTML = items.map(i => `
...
function showTable(title, headers, rows, footers = []) {
  ...
  document.getElementById("report-tfoot").innerHTML = footers.length ? `<tr>${footers.map(f=>`<td>${f}</td>`).join("")}</tr>` : "";
}
...
function daily(bills) {
  ...
  const rows = Object.entries(byDate).sort().map(([date, d]) =>
    [date, d.count, fmt(d.bags), fmt(d.weight)+" kg", fmtR(d.amount)]);
  ...
  showTable("Daily Report", ["Date","Bills","Bags","Weight","Amount"], rows,
    ["TOTAL", bills.length, "", "", fmtR(totals.reduce((s,d)=>s+d.amount,0))]);
}
...
function supplier(bills) {
  ...
  showTable("Supplier Report", ["Supplier","Village","Bills","Weight","Amount"], rows,
    ["TOTAL","", bills.length, "", fmtR(Object.values(bySupplier).reduce((s,d)=>s+d.amount,0))]);
}
...
function broker(bills) {
  ...
  showTable("Broker Report", ["Broker","Bills","Bags","Amount","Commission"], rows,
    ["TOTAL", bills.length, "", "", fmtR(Object.values(byBroker).reduce((s,d)=>s+d.commission,0))]);
}
...
function monthly(bills) {
  ...
  const rows = Object.entries(byMonth).sort().map(([key,d]) => {
    const [y,m] = key.split("-");
  ...
  showTable("Monthly Summary", ["Month","Bills","Bags","Weight","Amount"], rows,
    ["TOTAL", bills.length, "", "", fmtR(Object.values(byMonth).reduce((s,d)=>s+d.amount,0))]);
}
...
function payment(bills) {
  ...
  const totalDue = unpaid.reduce((s,b)=>s+(b["Final Total"]||0),0)
                 + partial.reduce((s,b)=>s+(b["Final Total"]||0)-(b.amountPaid||0),0);
  const rows = [...unpaid,...partial].sort((a,b)=>a.Date>b.Date?1:-1).map(b => [
    b["Serial No"], b.Date, b["Customer Name"],
    fmtR(b["Final Total"]),
    fmtR(b.amountPaid||0),
  ...
  showTable("Payment Report",
    ["Bill No","Date","Supplier","Total","Paid","Balance","Status"], rows);
}
