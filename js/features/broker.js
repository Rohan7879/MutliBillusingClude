      ...
      return `
        <div class="broker-card" onclick="viewBrokerDetails('${doc.id}')">
          ...
          <div class="broker-stats">
            <div><span class="bs-val">₹${Number(b.totalCommission||0).toLocaleString("en-IN")}</span><span class="bs-label">Total Commission</span></div>
            <div><span class="bs-val">₹${Number(b.paidAmount||0).toLocaleString("en-IN")}</span><span class="bs-label">Paid</span></div>
            <div><span class="bs-val" style="color:#dc3545;">₹${Number((b.totalCommission||0)-(b.paidAmount||0)).toLocaleString("en-IN")}</span><span class="bs-label">Balance Due</span></div>
  ...
  try {
    ...
    let html = `<h3 style="color:#005a9e;">🤝 ${broker.name}</h3>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px;">
        <div class="kpi-card"><div class="kpi-label">Total Commission</div><div class="kpi-value">₹${Number(broker.totalCommission||0).toLocaleString("en-IN")}</div></div>
        <div class="kpi-card"><div class="kpi-label">Paid</div><div class="kpi-value" style="color:#28a745;">₹${Number(broker.paidAmount||0).toLocaleString("en-IN")}</div></div>
        <div class="kpi-card"><div class="kpi-label">Balance Due</div><div class="kpi-value" style="color:#dc3545;">₹${Number((broker.totalCommission||0)-(broker.paidAmount||0)).toLocaleString("en-IN")}</div></div>
      </div>
      <table class="blt" style="width:100%;">
        <thead><tr><th>Bill No</th><th>Date</th><th>Supplier</th><th>Bags</th><th>Commission</th></tr></thead>
        <tbody>${billsSnap.docs.map(d => {
          const bill = d.data();
    ...
    Swal.fire({ html, width:"800px", confirmButtonText:"Close", confirmButtonColor:"#005a9e", showClass:{popup:"swal2-show"} });
  } catch(e) { console.error(e); }
  finally { hideLoading(); }
}
...
async function recordBrokerPayment(brokerId) {
  ...
  try {
    ...
    Swal.fire({ icon:"success", title:"Payment recorded!", toast:true, position:"top-end", showConfirmButton:false, timer:2000 });
    loadBrokerLedger();
  } catch(e) { console.error(e); }
}
...
async function updateBrokerCommission(billData) {
  ...
  try {
    ...
    if (doc.exists) {
      ...
      await brokerRef.set({
        name: brokerName, totalCommission: commission,
        paidAmount: 0, totalBills: 1, createdAt: Date.now()
      });
    }
  } catch(e) { console.warn("Broker update failed:", e); }
}
